...
export function joinLobby(socketId, playerName, lobbyCode, userId) {
  ...
  if (lobby.players.length >= 4) return { error: 'Room is full' };
...
const BOT_NAMES = ['Ricky', 'Shonda', 'Malik', 'Tanya', 'Dre', 'Keisha', 'Carlos', 'Jasmine'];
let botCounter = 0;
...
export function autoAssignTeams(lobbyCode) {
  const lobby = lobbies.get(lobbyCode);
  if (!lobby) return { error: 'Lobby not found' };
  if (lobby.players.length !== 4) return { error: 'Need exactly 4 players' };
